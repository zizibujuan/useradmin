...
CREATE  TABLE IF NOT EXISTS `DRIP_USER_INFO` (
  ...
  `LOCAL_ACCESS_TOKEN` VARCHAR(45) NULL COMMENT '本网站的access token',
  `OAUTH_ACCESS_TOKEN` VARCHAR(45) NULL COMMENT 'oauth access token',
  `ACCESS_TOKEN_EXPIRE_TIME` DATE NULL COMMENT 'access token过期时间',
  `REFRESH_TOKEN` VARCHAR(45) NULL COMMENT '续access token的刷新token',
